...
static bool parseLocation_Root(std::istringstream& locationStream, Location& location)
{
    ...
    if (!(root_path[0] == '/' || root_path.find("./") == 0))
    {
        std::cerr << "ERROR: LOCATION ROOT MUST START BY '/' OR './'" << std::endl;
        return false;
    }
...
static bool parseLocation_UploadDir(std::istringstream& locationStream, Location& location)
{
    ...
    if (!(upload_dir[0] == '/' || upload_dir.find("./") == 0))
    {
        std::cerr << "ERROR: WRONG UPLOAD_DIR MUST START BY '/' OR './'" << std::endl;
        return false;
    }
